...
namespace HuffmanCodeWithCSharp
{
    class ProcessMethods
    {
        ...
        public List<HuffmanNode> getListFromFile()
        {
            ...
            String currentDirectory = Environment.CurrentDirectory.ToString() + "\\text.txt";
            ...
            try
            {
                ...
                FileStream stream = new FileStream(currentDirectory, FileMode.Open, FileAccess.Read);
        ...
        public void PrintInformation(List<HuffmanNode> nodeList)
        {
            foreach (var item in nodeList)
                Console.WriteLine("Символ : {0} - Частота : {1}", item.symbol, item.frequency);
        }
        ...
        public void PrintfLeafAndCodes(HuffmanNode nodeList) 
        {
            ...
            if (nodeList.leftTree == null && nodeList.rightTree == null)
            {
                ...
                Global.kolByte += nodeList.code.Length;
                return;
            }
        ...
        public void OutputCodeResult(HuffmanNode nodeList)
        {
            ...
            if (nodeList.leftTree == null && nodeList.rightTree == null)
            {
                Global.list.Add(new SymbolsAndCodes() { symbol = nodeList.symbol.ToString(), code = nodeList.code.ToString(), frequency = nodeList.frequency.ToString() });
              //  Console.WriteLine("Символ : {0} -  Код : {1}", nodeList.symbol, nodeList.code);
                return;
            }

            OutputCodeResult(nodeList.leftTree);
            OutputCodeResult(nodeList.rightTree);
        }
